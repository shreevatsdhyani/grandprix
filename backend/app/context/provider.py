...
class CachedRaceContextProvider:
    ...
    def _path(self, session_id: str):
        # Session ids are generated by `make_session_id` and are already
        # restricted to [a-z0-9-], but this is a filesystem path built from a URL
        # parameter, so it gets sanitised anyway — same reasoning as
        # `store._result_path`.
...
def biometrics_path(session_id: str, driver: str):
    safe_s = "".join(ch for ch in session_id if ch.isalnum() or ch in "-_")
    safe_d = "".join(ch for ch in driver.upper() if ch.isalnum())
    return config.BIOMETRICS_DIR / f"{safe_s}-{safe_d}.json"
